...
class Admin_group extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $extra['buttons'] = array(
            array(
                ...
                'url' => 'admin/'.$this->module_name.'/group/edit/-entry_id-'
            ),
            array(
                'label' => lang('global:delete'),
                'url' => 'admin/'.$this->module_name.'/group/delete/-entry_id-',
                'confirm' => true
            )
        );
        ...
        $this->streams->cp->entries_table('group', 'ug', 15, 'admin/'.$this->module_name.'/group/index', true, $extra);
    }
    ...
    {
        $extra = array(
            'return' => 'admin/'.$this->module_name.'/group',
            'success_message' => lang('ug:submit_success'),
            'failure_message' => lang('ug:submit_failure'),
            'title' => 'lang:ug:creategroup',
         );

        $this->streams->cp->entry_form('group', 'ug', 'new', null, true, $extra);
    }
    ...
    public function edit($id = 0)
    {
        $extra = array(
            ...
            'success_message' => lang('ug:submit_success'),
            'failure_message' => lang('ug:submit_failure'),
            'title' => 'lang:ug:editgroup'
        );

        $this->streams->cp->entry_form('users', 'ug', 'edit', $id, true, $extra);
    }
    ...
    {
        $this->streams->entries->delete_entry($id, 'group', 'ug');
        $this->session->set_flashdata('error', lang('ug:deleted'));
 
        redirect('admin/'.$this->module_name.'/group/');
    }
